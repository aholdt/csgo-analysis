...
export class TeamGameStats {
  constructor(teamRoundStats: TeamRoundStats[]) {
    ...
    this.roundWinPct = (teamRoundStats.reduce((total, next) => total + (next.roundWon ? 1 : 0), 0) / teamRoundStats.length) * 100;

    this.multiKillPct = (teamRoundStats.reduce((total, next) => total + next.multiKills, 0) / teamRoundStats.length) * 100;
    this.openingKillPct = teamRoundStats.reduce((total, next) => total + next.openingKills, 0) * 100;
